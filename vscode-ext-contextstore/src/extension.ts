...
import { FolderTreeDataProvider, FolderItem, NoteItem } from './tree/FolderTree';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.commands.registerCommand('contextstore.signUp', async () => {
  ...
  const client = sb();
    ...
    vscode.commands.registerCommand('contextstore.deleteFolder', async (item: FolderItem) => {
      const client = sb();
      if (!client || !userId) return;

      const confirm = await vscode.window.showWarningMessage(
        `Delete folder "${item.label}" and all its notes?`,
        { modal: true },
        'Delete'
      );
    ...
    vscode.commands.registerCommand('contextstore.createNote', async (item: FolderItem) => {
      const client = sb();
      if (!client || !userId) return;
    ...
    vscode.commands.registerCommand('contextstore.openNote', async (item: NoteItem) => {
      const client = sb();
      if (!client || !userId) return;
    ...
    vscode.commands.registerCommand('contextstore.editNote', async (item: NoteItem) => {
      const client = sb();
      if (!client || !userId) return;
    ...
    vscode.commands.registerCommand('contextstore.copyNote', async (item: NoteItem) => {
    const client = sb();
      if (!client || !userId) return;
    ...
    vscode.commands.registerCommand('contextstore.deleteNote', async (item: NoteItem) => {
      const client = sb();
      if (!client || !userId) return;
  ...
  if (!userId) {
    vscode.window.showInformationMessage(
      'Welcome to ContextStore! Sign in or sign up to sync your notes.',
      'Sign In',
      'Sign Up'
    ).then(selection => {
