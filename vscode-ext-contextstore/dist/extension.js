...
// src/providers/ContextStoreProvider.ts
...
// src/providers/ContextStoreProvider.ts
var FolderItem = class extends vscode.TreeItem {
  constructor(folder) {
...
var NoteItem = class extends vscode.TreeItem {
  constructor(note) {
    const title = note.text.split("\n")[0] || "(untitled)";
    super(title.slice(0, 60), vscode.TreeItemCollapsibleState.None);
    this.note = note;
    this.contextValue = "note";
    this.iconPath = new vscode.ThemeIcon("note");
    this.description = new Date(note.timestamp).toLocaleDateString();
    this.tooltip = title;
    this.command = {
...
var ContextStoreProvider = class {
  ...
  async getChildren(element) {
    ...
    if (!user) return [];
    if (!element) {
      const { data: folders, error } = await supabase.from("folders").select("*").eq("user_id", user.id).order("id", { ascending: false });
      if (error) {
    ...
    if (element instanceof FolderItem) {
      const { data: notes, error } = await supabase.from("notes").select("*").eq("user_id", user.id).eq("folder_id", element.folder.id).order("id", { ascending: false });
      if (error) {
        console.error("Error fetching notes:", error);
        return [];
      }
      return (notes || []).map((n) => new NoteItem(n));
    }
    return [];
  }
  getParent(element) {
  ...
  async createNote(folderId, text) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return false;
  ...
  async updateNote(noteId, text) {
    const { error } = await supabase.from("notes").update({ text, timestamp: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", noteId);
    if (!error) this.refresh();
    return !error;
  }
...
function activate(context) {
  const providers = new ContextStoreProvider();
  vscode2.window.registerTreeDataProvider("contextstoreview", providers);
  context.subscriptions.push(
    vscode2.commands.registerCommand("contextstore.refresh", () => providers.refresh())
  );
}
function deactivate() {
}
